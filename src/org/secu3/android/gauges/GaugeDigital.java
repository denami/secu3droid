...
public class GaugeDigital extends BaseGauge {
	...
	public void load(BaseGameActivity activity) throws IOException {
		...
		this.secu_font = FontFactory.createFromAsset(activity.getFontManager(),activity.getTextureManager(), 512, 512, TextureOptions.BILINEAR, activity.getAssets(), fontName, fontSize, true, color);
		this.secu_font.load();
	}
	...
	public void attach(Scene scene, VertexBufferObjectManager vertexBufferObjectManager) {
		...
		text = new Text (getX(),getY(),secu_font,String.format(format, value),15,vertexBufferObjectManager);
